...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.finance.treasurychain.com"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.trustee.treasurychain.com"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.operations.treasurychain.com"
    PORT=11051
  elif [ $ORG -eq 4 ]; then
    HOST="peer0.audit.treasurychain.com"
    PORT=13051
  else
...
updateAnchorPeer() {
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.treasurychain.com -c $CHANNEL_NAME -f ${TEST_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
