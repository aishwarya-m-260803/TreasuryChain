...
# This script brings up the TreasuryChain Hyperledger Fabric network.
# It supports Fabric CA-based crypto generation and network lifecycle
# operations for Finance, Trustee, Operations, Audit and OrdererOrg.
#
# Prepending $PWD/../bin to PATH to ensure we are picking up the correct binaries.
# The location this script is run from is critical. To ease this, get the directory
# this script is actually in and infer location from there.
...
# Versions of fabric known not to work with this network
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $NONWORKING_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      fatalln "Local Fabric binary version of $LOCAL_VERSION is not supported."
    fi

    infoln "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      fatalln "Fabric Docker image version of $DOCKER_IMAGE_VERSION is not supported."
    fi
  done

  if [ "$CRYPTO" != "Certificate Authorities" ]; then
    fatalln "Only Fabric CA is supported in this network. Use -ca to enable Fabric CA."
  fi
...
# material that will define that organization on the network. Fabric CA servers are
# used to generate MSP certificates and enroll identities for Finance, Trustee,
# Operations, Audit, and OrdererOrg.
#
# The configuration for Fabric CA servers is in the "organizations/fabric-ca"
# directory. The registerEnroll.sh script creates the required MSP directories
# under "organizations/peerOrganizations" and "organizations/ordererOrganizations".

# Create Organization crypto material using Fabric CA
function createOrgs() {
  ...
  # Make sure CA configuration and certificates are available
  while :
  do
    if [ ! -f "organizations/fabric-ca/finance/ca-cert.pem" ]; then
      sleep 1
    else
      break
    fi
  ...
  while [[ $rc -ne 0 && $COUNTER -lt $MAX_RETRY ]]; do
    sleep 1
    set -x
    fabric-ca-client getcainfo -u https://admin:adminpw@localhost:7054 --caname FinanceCA --tls.certfiles "${PWD}/organizations/fabric-ca/finance/ca-cert.pem"
    res=$?
    { set +x; } 2>/dev/null
    rc=$res
    COUNTER=$((COUNTER + 1))
  ...
  infoln "Creating Finance Org Identities"
  createFinanceOrg

  infoln "Creating Trustee Org Identities"
  createTrusteeOrg

  infoln "Creating Operations Org Identities"
  createOperationsOrg
...
# of the file. This profile defines an application channel consisting of
# Finance, Trustee, Operations, and Audit organizations.
...
# file for creating the TreasuryChain network is "compose-test-net.yaml" in the
# ``compose`` folder. This file defines the environment variables and file mounts
# that point to the crypto material and genesis block that were created earlier.
...
# call the script to create the channel, join the peers of the configured organizations,
...
function networkDown() {
  ...
  COMPOSE_CA_FILES="-f ${COMPOSE_FILE_CA}"
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/finance organizations/fabric-ca/trustee organizations/fabric-ca/operations organizations/fabric-ca/audit organizations/fabric-ca/ordererOrg'
